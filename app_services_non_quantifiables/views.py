...
import logging
from services.translation import build_translated_fields, TRANSLATABLE_FIELDS_SNQ
...
class AAOView(GenericAPIView):
    ...
    def _get_translation_kwargs(self, validated_data):
        try:
            translated = build_translated_fields(validated_data, fields=TRANSLATABLE_FIELDS_SNQ)
            if not translated:
                return {}
...
def generer_pdf(request, project_id, mode='inline'):
    ...
    template = loader.get_template('app_services_non_quantifiables/dao_services_non_quantifiables.html')
...
def apercu_marche_services_non_quantifiables(request, project_id):
    return generer_pdf(request, project_id, mode='inline')


def telecharger_marche_services_non_quantifiables(request, project_id):
